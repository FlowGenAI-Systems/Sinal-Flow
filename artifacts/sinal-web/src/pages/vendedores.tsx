...
export default function Vendedores() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetch(`/api/sellers?days=${days}`, { credentials: "same-origin" })
      .then((r) => {
  ...
  const cols: { key: SortKey; label: string; num: boolean }[] = [
    { key: "vendedor", label: "Vendedor", num: false },
    { key: "tempo_resp_min", label: "Tempo de resposta", num: true },
    { key: "conversas_dia", label: "Conversas/dia", num: true },
    { key: "pendencias", label: "Pendências", num: true },
  ];

  const fmtTempo = (m: number) =>
    !m || m <= 0 ? "—" : m < 60 ? `${Math.round(m)} min` : `${(m / 60).toFixed(1)} h`;
    ...
    <div className="p-6">
      <div className="mb-5 flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-slate-500">Comparativo do time — últimos {days} dias</p>
        </div>
        <select
          value={days}
            ...
            <tr className="border-b border-slate-200 bg-slate-50">
              {cols.map((c) => (
                <th
                  ...
                  className={`cursor-pointer select-none px-4 py-3 font-medium text-slate-600 hover:text-slate-900 ${
      ...
      <p className="mt-3 text-xs text-slate-400">
        Tempo de resposta: mediana em horário comercial (seg–sex, 8h–17h, fuso de São Paulo).
        Conversas/dia: média de contatos distintos por dia. Pendências: conversas cuja última mensagem é do cliente.
      </p>
    </div>
  );
}
